...
class DNN:
    def __init__(self, env, weight_file, max_episode = 5000, INITIAL_EPSILON = 0.2, FINAL_EPSILON = 0.0001):
        ...
        self.s = tf.compat.v1.placeholder(tf.float32, [None, self.state_num], name ='s')
        ...
        self.s_target = tf.compat.v1.placeholder(tf.float32, [None, self.state_num], name ='s_target')
    ...
    def variable_b(self, shape, initial = 0.01):
        b = tf.compat.v1.get_variable('b', shape = shape, initializer = tf.compat.v1.constant_initializer(initial))    
        return b
    ...
    def save_params(self):
        ...
        io.savemat(self.weight_file, dict_name)
...
#                print(var.name, var.shape)
                ...
                update.append(tf.compat.v1.assign(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name),tf.compat.v1.constant(np.reshape(theta[var.name],var.shape))))
        except:
            print('fail dqn')
            update=[]
        return update
...
class DQN:
    def __init__(self, sess, dnn, learning_rate = 1e-3):
        ...
        self.a_hat = tf.compat.v1.argmax(self.q_hat, 1)
        ...
        self.loss = tf.compat.v1.nn.l2_loss(self.y - self.r)
